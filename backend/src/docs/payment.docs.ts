...
registry.registerPath({
  ...
  description: [
    ...
    '',
    '**PICA 2A consultation credit (Phase 2A only):** before the subscription quota path, the BE checks for the user\'s oldest unconsumed, unexpired `Phase2ACredit` (granted by `ConsultationTier.freeP2ARuns` on consultation confirm). If found, returns `free: true` with `paymentMethod = "consultation-credit"` on the resulting Payment row — no Paystack call, no quota consumption.',
    '',
    '**Subscription quota short-circuit:** falls through to the active subscription when no credit applies. Phase 2A consumes 1 quota unit per call; Phase 2B consumes one per pillar.',
    '',
    '**Phase 2B bundle discount cap:** the percentage applies only to the first `phase2bDiscountMaxPillars` selected pillars (by selection order); extras pay full price. So the dollar discount plateaus at the cap instead of growing with additional pillars beyond it.',
    '',
