...
export const getPhase2AQuestions = asyncHandler(
	async (req: Request, res: Response) => {
		if (!req.user?.id) {
...
export const getPhase2BQuestions = asyncHandler(
	async (req: Request, res: Response) => {
		if (!req.user?.id) {
			throw new AppError('Authentication required', UNAUTHORIZED)
		}
...
export const getAllPillars = asyncHandler(
	async (req: Request, res: Response) => {
		if (!req.user?.id) {
			throw new AppError('Authentication required', UNAUTHORIZED)
		}
