...
export const startPhase2A = asyncHandler(async (req: Request, res: Response) => {
...
export const startPhase2B = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user?.id || req.user?.role !== 'USER') {
    throw new AppError('Authentication required', UNAUTHORIZED);
  }
...
export const getMyPhase2BPillars = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user?.id || req.user?.role !== 'USER') {
    throw new AppError('Authentication required', UNAUTHORIZED);
  }
...
export const getSessionResponses = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user?.id || req.user?.role !== 'USER') {
    throw new AppError('Authentication required', UNAUTHORIZED);
  }
  const { sessionId } = assessmentSessionParams.parse(req.params);
