...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
      ...
      <Link
        ...
        onClick={() => setSidebarOpen(false)}
          className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors relative ${
            active
              ? (d ? "text-white bg-blue-500/20" : "text-blue-700 bg-blue-50")
  ...
  if (!authChecked) {
    return (
      <div className={`min-h-screen ${d ? 'bg-[#111318] text-white' : 'bg-white text-gray-900'} flex items-center justify-center`}>
        <div className={`text-sm ${d ? 'text-gray-400' : 'text-gray-600'}`}>Checking admin session...</div>
      </div>
    );
  }

  if (pathname === "/admin/login") {
    return <div className={`${d ? 'bg-[#111318] text-white' : 'bg-white text-gray-900'} min-h-screen w-full`}>{children}</div>;
  }
  ...
  return (
    ...
    >
      {/* Top Nav */}
      <header className={`flex items-center justify-between px-6 py-4 ${d ? 'bg-[#111318]/80 border-white/5' : 'bg-white/95 border-gray-200'} backdrop-blur-md border-b sticky top-0 z-50`}>
        <div className="flex items-center gap-8">
          {/* Mobile hamburger */}
          <button
            className={`lg:hidden ${d ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'}`}
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className={`${d ? 'text-white' : 'text-gray-900'} font-bold text-lg tracking-wide hidden sm:block`}>
                Beauvision
              </span>
            </div>
            <div className={`h-4 w-px ${d ? 'bg-white/20' : 'bg-gray-300'} hidden sm:block`}></div>
            <Link href="/admin" className={`${d ? 'text-white' : 'text-gray-900'} font-bold text-xl tracking-wide hidden sm:block`}>
              PICA
            </Link>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Sidebar */}
        <aside
          className={`fixed lg:static top-0 left-0 bottom-0 z-50 w-60 ${d ? 'bg-[#161925]/80 border-white/5' : 'bg-white/95 border-gray-200'} backdrop-blur-md border-r flex flex-col py-6 px-3 transition-transform duration-300 ${
        ...
        >
          <nav className="space-y-1 flex-1 overflow-y-auto">
            ...
            <div className={`my-4 h-px ${d ? 'bg-white/5' : 'bg-gray-200'} mx-4`} />
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors w-full ${d ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'} mt-auto`}
          >
        ...
        {/* Main content */}
        <main className={`flex-1 overflow-y-auto overflow-x-hidden relative ${d ? 'bg-black/40' : 'bg-gray-50/60'}`}>
          {/* Mobile hamburger button */}
          <button
            className={`lg:hidden fixed top-4 left-4 z-30 ${d ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'}`}
            onClick={() => setSidebarOpen(!sidebarOpen)}
          >
            {sidebarOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>
          ...
          <div className={`absolute top-0 left-0 w-full h-96 bg-gradient-to-b ${d ? 'from-blue-900/10' : 'from-blue-50/30'} to-transparent pointer-events-none`} />
      ...
      {logoutModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
          <div className={`${d ? 'bg-[#1a1f2e] border-white/10' : 'bg-white border-gray-200'} border rounded-lg p-6 max-w-sm`}>
            <h2 className={`text-lg font-bold ${d ? 'text-white' : 'text-gray-900'} mb-2`}>Leave Admin Dashboard?</h2>
            <p className={`${d ? 'text-gray-400' : 'text-gray-600'} mb-6`}>Are you sure you want to leave?</p>
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => setLogoutModal(false)}
                className={`px-4 py-2 rounded-lg text-sm font-medium ${d ? 'text-gray-400 hover:bg-white/5' : 'text-gray-600 hover:bg-gray-100'} transition-colors`}
              >
                No
              </button>
              <button
