...
export default function QuestionBankPage() {
  ...
  // Score Labels state
  const [mode, setMode] = useState<"questions" | "labels" | "knockouts">("questions");
  ...
  const knockoutOptions = useMemo(() => {
    return questions.flatMap(q => 
      q.options
        .filter(o => o.riskType === 'KNOCKOUT')
        .map(o => ({ question: q, option: o }))
    );
  }, [questions]);
    ...
    <div className="mx-auto max-w-[1400px] space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            type="button"
            onClick={() => setMode(mode === "questions" ? "labels" : mode === "labels" ? "knockouts" : "questions")}
          ...
          >
            ...
            {mode === "questions" ? "Overall Recommendations" : mode === "labels" ? "Knockout View" : "Questions View"}
          </button>
          <button
            type="button"
            onClick={() => {
      ...
      <div className="w-full space-y-4">
        ...
        {mode === "questions" ? (
          ...
          loading ? (
            ...
            <div className="rounded-xl border border-white/5 bg-[#1C1F2E] p-8 text-center text-sm text-gray-500">
              No knockout options found in the current question set.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {knockoutOptions.map(({ question, option }) => (
                <button
                  type="button"
                  key={option.id}
                  onClick={() => {
                ...
                >
                  <div className="w-full">
                    ...
                    <div className="space-y-2">
                      <p className="line-clamp-2 text-xs leading-relaxed text-amber-200/80">
                        <span className="font-semibold">Observation:</span> {option.observation}
                      </p>
                      <p className="line-clamp-2 text-xs leading-relaxed text-emerald-200/80">
                        <span className="font-semibold">Rec:</span> {option.recommendation}
                      </p>
                    </div>
                  </div>
                  <div className="mt-4 pt-3 border-t border-white/5 flex items-center gap-2 text-[10px] font-semibold text-gray-500 w-full uppercase tracking-wider">
                    <AlertCircle className="h-3.5 w-3.5 text-red-400" />
                    Click to edit question & options
                  </div>
                </button>
              ))}
            </div>
